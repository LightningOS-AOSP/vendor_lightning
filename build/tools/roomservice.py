...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from LightningOS-Devices Github." % device)

repositories = []

if not depsonly:
    queries = [
        f"device_{device} user:LightningOS-Devices",
        f"android_device_{device} user:LightningOS-Devices",
    ]
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if "/" in repo_name_raw:
            ...
            project_name = f"LightningOS-Devices/{repo_name_raw}"
...
def get_default_or_fallback_revision(repo_name):
    ...
    try:
        result = subprocess.run(
            ["git", "ls-remote", "-h", f"https://github.com/LightningOS-Devices/{repo_name}"],
...
url = "https://raw.githubusercontent.com/LightningOS-Devices/official_devices/refs/heads/17/buildDevices.json"
